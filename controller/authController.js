...
export const updateProfileController = async (req, res) => {
  try {
    ...
    const updatedUser= await userModel.findByIdAndUpdate(req.user._id,{},{name:name||user.name,password:password||user.password,phone:phone||user.phone,address:address||user.address,new:true});
    res.status(200).send({message:"Profile upadted successully",success:true,updatedUser})
  } catch (error) {
    console.log(error);
    res.status(400).send({success:false,message:"error while updating profile"})
  }
};
